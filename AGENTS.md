...
**Short-form is ALWAYS part of every idea session (user directive 2026-09-04).** Every trend sweep and video-idea delivery must include BOTH a long-form recommendation AND a short-form (YouTube Shorts / TikTok / Reels, 30-60s vertical) recommendation. Shorts are cut from existing long-video footage whenever possible (zero new shooting) and exist to feed the long video + ride same-day news. Deliver as a compact package: hook text overlay, beat-by-beat cut list with source timestamps, platform captions (Shorts/TikTok/Reels), hashtags, and CTA.

**Commands MUST come from official documentation.** Before writing any install/setup command into a video package or blog post, fetch the tool's official docs (vendor docs, GitHub README/docs, release notes) and verify the exact commands, env vars, and requirements — never reconstruct commands from memory or inference. Cite the official source in the package (link + "verified against official docs"). If an official page can't be fetched, mark the command clearly as UNVERIFIED rather than guessing. Applies to blog posts too: transcript + official docs together.
